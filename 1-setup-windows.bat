...
echo     $tmp = Join-Path $env:TEMP ^("$^($name -replace '[^a-zA-Z0-9]', ''^)Setup.exe"^)>>"%SCRIPT%"
...
echo     if ^(Test-Path $installedPath^) { Write-Host "$name already installed, skipping..." -ForegroundColor Yellow; return }>>"%SCRIPT%"
echo     Write-Host "Installing $name..." -ForegroundColor Cyan>>"%SCRIPT%"
echo     $tmp = Join-Path $env:TEMP ^("$^($name -replace '[^a-zA-Z0-9]', ''^)Setup.msi"^)>>"%SCRIPT%"
echo     if ^(Test-Path $tmp^) { Remove-Item $tmp -Force -ErrorAction SilentlyContinue }>>"%SCRIPT%"
echo     Invoke-WebRequest -Uri $url -OutFile $tmp>>"%SCRIPT%"
...
echo Install-DirectExe "Discord Canary" "https://discord.com/api/downloads/distributions/app/installers/latest?channel=canary&platform=win&arch=x64" "-s" "$env:LOCALAPPDATA\DiscordCanary">>"%SCRIPT%"
echo Install-DirectMsi "Chrome Remote Desktop" "https://dl.google.com/edgedl/chrome-remote-desktop/chromeremotedesktophost.msi" "${env:ProgramFiles(x86)}\Google\Chrome Remote Desktop">>"%SCRIPT%"
