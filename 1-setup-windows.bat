...
echo     Invoke-WebRequest -Uri $url -OutFile $tmp>>"%SCRIPT%"
...
echo     try {>>"%SCRIPT%"
echo         $proc = Start-Process -FilePath "wsl.exe" -ArgumentList "-l -q" -NoNewWindow -PassThru>>"%SCRIPT%"
echo         if ($proc.WaitForExit(10000)) {>>"%SCRIPT%"
echo             if ($proc.ExitCode -eq 0) { $wslReady = $true }>>"%SCRIPT%"
echo         } else {>>"%SCRIPT%"
echo             $proc ^| Stop-Process -Force -ErrorAction SilentlyContinue>>"%SCRIPT%"
echo             Write-Host "WSL check timed out; skipping WSL setup." -ForegroundColor Yellow>>"%SCRIPT%"
echo         }>>"%SCRIPT%"
echo     } catch { Write-Host "WSL check failed: $($_.Exception.Message)" -ForegroundColor Yellow }>>"%SCRIPT%"
echo }>>"%SCRIPT%"
...
echo     }>>"%SCRIPT%"
echo } else { Write-Host "WSL not ready yet - skipping WSL setup." -ForegroundColor Yellow }>>"%SCRIPT%"
