    ...
    try {
      ...
      for (let attempt = 1; attempt <= 8; attempt++) {
        ...
        try {
          ...
          const passed = !hitAccessLogin && !hitCloudflareError && (status === 200 || redirected || (status >= 300 && status < 400));
          lastResult = {
            Name: name,
            Url: url,
            Status: status,
            ...
            Passed: passed,
            Detail: `HTTP ${status} -> ${finalUrl}${redirected ? ` (redirected ${redirectCount}x)` : ''}${hitAccessLogin ? ' (Cloudflare Access login page)' : ''}${hitCloudflareError ? ' (Cloudflare error page)' : ''}`,
          };
